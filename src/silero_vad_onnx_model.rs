...
//saying this is unused?
use anyhow::Error;

pub struct SileroVadOnnxModel {

    model_location: String,
    sample_rate: ndarray::prelude::ArrayBase<ndarray::OwnedRepr<i64>, ndarray::prelude::Dim<[usize; 1]>>,
    h: ndarray::prelude::ArrayBase<ndarray::OwnedRepr<f32>, ndarray::prelude::Dim<[usize; 3]>>,
    c: ndarray::prelude::ArrayBase<ndarray::OwnedRepr<f32>, ndarray::prelude::Dim<[usize; 3]>>,
    vad_threshold: f32,
    session: ort::Session,

}

impl SileroVadOnnxModel {
    ...
    pub fn run_vad(&mut self, audio_data: Vec<f32>) -> Result<Vec<bool>, anyhow::Error> {
        ...
        for chunk in audio_chunks {
            ...
            let chunk_outputs: &ort::SessionOutputs<'_> = &self.session.run(inputs![input_chunk, self.sample_rate.clone(), self.h.clone(), self.c.clone()]?)?;
...
mod tests {
    ...
    fn test_struct() {
        ...
        let mut silero_model = SileroVadOnnxModel::new(("/Users/oliver.pikett/Documents/GitHub/silero-rs/files/silero_vad.onnx").to_string()).unwrap();
